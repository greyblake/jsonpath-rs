...
fn test_filter_with_absolute_condition() {
  assert_jsonpath_str!("$.store.books[?($.store.books[2].category == 'fiction')].title", ["Sayings of the Century", "Sword of Honour", "Moby Dick", "The Lord of the Rings"]);
  assert_jsonpath_str!("$.store.books[?($.store.books[2].category != 'reference')].title", ["Sayings of the Century", "Sword of Honour", "Moby Dick", "The Lord of the Rings"]);
}
